...
const SITE_URL = "https://gin-check-masks-start-a9pc.vercel.app/";
...
export const app = new Frog({
  ...
  hub: {
    ...
    fetchOptions: {
      headers: {
        "x-airstack-hubs": "1daf57d47cf4d4e95876dea50bca257d0",
      }
    }
  }
}).use(
...
function Content(weeklyAllowance:string, remainingAllowance:string, masks:string, rank:string) {
  return <Row paddingLeft="64" paddingTop="32" height="5/7"> 
          <Columns gap="8" grow> 
            <Column width="3/7"> 
              <Rows gap="8" grow>
                ...
                <Row paddingLeft="12" height="2/7" > 
                  <Columns gap="8" grow> 
                    <Column alignVertical='bottom' width="2/7"> <Text>Allowance: </Text> </Column>
                    <Column width="4/7"> <Text align='right' color="main" weight="900" size="20"> { weeklyAllowance } </Text> </Column>
                  </Columns>
                  <Columns gap="8" grow> 
                    <Column alignVertical='bottom' width="2/7"> <Text>Remaining: </Text> </Column>
            ...
            <Column width="3/7">
              <Rows gap="8" grow>
                ...
                <Row paddingLeft="12" height="2/7" > 
                  <Columns gap="8" grow> 
                    <Column alignVertical='bottom' width="2/7"> <Text>Balance: </Text> </Column>
                    <Column width="4/7"> <Text color="main" align='right' weight="900" size="20"> { masks } </Text> </Column>
                  </Columns>
                  <Columns gap="8" grow> 
                    <Column alignVertical='bottom' width="2/7"> <Text>Rank: </Text> </Column>
                    <Column width="4/7"> <Text color="main" align='right' weight="900" size="20">{ rank }</Text> </Column>
                  </Columns>
                </Row>
              </Rows>
            </Column>
...
app.frame('/:fid/:secret', async (c) => {
  ...
  var test = await fetch("https://build.top/api/nominations?from=0x33041027dd8F4dC82B6e825FB37ADf8f15d44053", {method:"GET"})
  console.log(JSON.parse(await test.text()) || {})
